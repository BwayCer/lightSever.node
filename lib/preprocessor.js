...
pushPreprocessor(
    ...
    /* async */ function tool(limitOwn, fnDone) {
        let localBabelrcPath = path.join(limitOwn.rootPath, '/.babelrc');
        let dirPath = path.dirname(limitOwn.url);
        ...
        Promise.resolve(null)
            .then(function () {
                ...
                let asyncList = [
                    ...
                    new Promise(function (resolve, reject) {
                        if (!fs.existsSync(localBabelrcPath)) {
                            resolve('');
                            return;
                        }

                        fs.readFile(localBabelrcPath, 'utf8', function (err, data) {
                            if (err) reject(err);
                            else resolve(data);
                        });
                    }),
            ...
            .then(function ([babelrcCode, sourceEsCode]) {
                let presets = babelrcCode === '' ? babelrcCode : babelOption_presets;
                console.log(presets, '\n', babelrcCode, '\n', sourceEsCode);
